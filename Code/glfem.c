...
void glfemReshapeWindows(meshPoint **thePoint, int w, int h, int LengthNoBigTriangle)
{
    ...
    double maxY  = 0;
    int i = 0;
    if (LengthNoBigTriangle > 0)
    {
		for(i=3;i< LengthNoBigTriangle;i++)
		{
	 	if(thePoint[i]->x>maxX){
    		maxX = thePoint[i]->x;}
    	if(thePoint[i]->x<minX){
    		minX = thePoint[i]->x;}
    	if(thePoint[i]->y>maxY){
    		maxY = thePoint[i]->y;}
    	if(thePoint[i]->y<minY){
    		minY = thePoint[i]->y;}
		}
	}
	else{
...
void glfemPlotMesh(meshPoint **thePoint, int *A, int *B, int *C, int lengthTot)
{
    ...
    for (i = 0; i < lengthTot; ++i) {
            xLoc[0] = thePoint[A[i]]->x;            
            xLoc[1] = thePoint[B[i]]->x;
            xLoc[2] = thePoint[C[i]]->x;
            yLoc[0] = thePoint[A[i]]->y; 
            yLoc[1] = thePoint[B[i]]->y; 
            yLoc[2] = thePoint[C[i]]->y;                  
